...
The helper script creates a new branch and a matching checkout under `.worktrees/`, using a lowercase hexadecimal directory name derived from the branch name. `.worktrees/` is Git-ignored on purpose, so worktree contents stay out of the main checkout.
