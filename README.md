...
Run the command from the branch you want to branch off from. If you run it in the repository root on `main`, the new worktree is created under the root `.worktrees/`. If you run it inside an existing worktree, the child worktree is created in that worktree's adjacent `.worktrees` storage instead of inside the parent checkout, so removing the parent worktree does not delete the child. The helper fails fast if `.worktrees/` is not ignored, if the branch already exists, or if the target worktree path already exists.
