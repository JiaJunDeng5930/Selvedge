...
pub(crate) fn read_chatgpt_auth_config() -> Result<ChatgptAuthConfig, ChatgptLoginError> {
    ...
    let config = ChatgptAuthConfig {
        ...
        // @constraint selvedge.login.config.valid.settings_type.read ChatGPT login config reading returns validation failures for non-string known provider settings.
        expected_workspace_id: setting_string(&settings, "expected_workspace_id")
            .map_err(validation_config_error)?,
    };
    // @constraint selvedge.login.config.valid.read ChatGPT login config reading returns validation failures for invalid provider settings.
...
// @constraint selvedge.login.config.valid.settings_type ChatGPT login provider settings with known string keys must be TOML strings.
fn setting_string(
...
fn validation_config_error(reason: String) -> ChatgptLoginError {
    ChatgptLoginError::Config(selvedge_config::ConfigError::ValidationFailed(reason))
}
