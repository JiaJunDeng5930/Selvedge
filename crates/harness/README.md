...
freshness_fingerprint: c73d1fa1a545c4742cb316cf8bea7b976eb63fea
-->
...
One `CommandEnvironmentManager` is shared by the server's tool executor. Both `exec_cmd` and ordinary `fork_task` acquire the referenced environment's exclusive lease and keep it inside `ToolExecutionCompletion` until core atomically commits the checkpoint and outer output. Fork's `environment` is `shared` by default, `copy` for independent state, or `new` for base capabilities. Script-created children remain durable but cannot execute until outer completion; their final copy includes changes made after the fork call. An interrupted admitted predecessor is explicitly recovered before another task can use its environment.
