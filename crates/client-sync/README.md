...
Only the current `ClientSessionIdentity` may deliver its builder result. A new session for the same client aborts the old builder. A duplicate start for the current session is ignored. `CancelHydration` aborts the matching builder. `Shutdown` aborts and joins every builder before the loop stops.
...
flowchart TD
  ...
  Loop -->|StartHydration for new client session arrives| ReserveHydration
  Loop -->|StartHydration matches current session identity| IgnoreDuplicate
  Loop -->|CancelHydration matches current session identity; abort builder| Cancel
  Loop -->|CancelHydration misses current session| Loop
  ...
  Build -->|new session replaces current session; abort old builder| ReserveHydration
  ReserveHydration -->|events mailbox send fails| Fatal
  Build -->|builder returns snapshot for current session| DeliverSnapshot
  Build -->|builder returns error for current session| DeliverFailure
  Build -->|builder result session identity is no longer current| DropStale
